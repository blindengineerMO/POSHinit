  ...
  try {
    ...
    for (const machine of selectedMachines.value) {
      try {
        ...
        const result = await store.testMachineCandidate({ target: machine.ipAddress || machine.name, credentialId: wizard.credentialIds[machine.id], port: 5985 })
...
async function importVerified() {
  ...
  error.value = ''
  try {
    const verifiedIds = results.value.filter((result) => result.ok).map((result) => result.machine.id)
    await store.importVmwareSelection(wizard.connectorId, verifiedIds, wizard.credentialIds)
    imported.value = true
    step.value = 4
  } catch (requestError) {
    error.value = requestError.message
...
<template>
  <FloatingWindow v-model="open" title="Import VMware Virtual Machines" :width="760" :start-x="185" :start-y="92">
    <div class="wizard-shell">
      <div class="wizard-steps"><button v-for="item in [{ id: 1, label: 'Select inventory', icon: 'mdi-view-list-outline' }, { id: 2, label: 'Assign access', icon: 'mdi-key-outline' }, { id: 3, label: 'Verify nodes', icon: 'mdi-lan-check' }, { id: 4, label: 'Import summary', icon: 'mdi-check-decagram-outline' }]" :key="item.id" type="button" :class="['wizard-step', { active: step === item.id, complete: step > item.id }]" :disabled="item.id > step" @click="step = item.id"><v-icon :icon="item.icon"/><span>{{ item.label }}</span></button></div>
