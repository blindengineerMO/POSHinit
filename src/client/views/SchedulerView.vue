...
async function runNow() { runResults.value = []; runOutput.value = []; dispatchId.value = ''; lastDispatchId.value = ''; dispatchRunning.value = true; runWindow.value = false; resultWindow.value = true; try { await store.streamRunScripts(manualRunDraft, (event) => { if (event.type === 'dispatch') { dispatchId.value = event.dispatchId; lastDispatchId.value = event.dispatchId } if (event.type === 'stdout' || event.type === 'stderr') runOutput.value.push({ ...event, id: `${event.sequence || event.executionId}-${runOutput.value.length}` }); if (event.type === 'complete') runResults.value.push(event.execution) }) } finally { dispatchRunning.value = false; dispatchId.value = '' } }
async function cancelRunNow() { if (dispatchId.value) await store.cancelRunDispatch(dispatchId.value) }
async function downloadRunOutput() { if (lastDispatchId.value) await store.downloadDispatchOutput(lastDispatchId.value) }
</script>
...
<template>
  <div class="planner-workspace">
    ...
    <FloatingWindow v-model="runWindow" title="Dispatch Runbook" :width="480" :start-x="450" :start-y="120"><div class="window-form"><v-select v-model="manualRunDraft.scriptIds" :items="scripts" item-title="name" item-value="id" label="Runbooks" multiple chips/><v-select v-model="manualRunDraft.machineIds" :items="machines" item-title="name" item-value="id" label="Execution targets" multiple chips/><div class="time-grid"><v-text-field v-model.number="manualRunDraft.timeoutSeconds" type="number" min="10" max="86400" label="Per-target timeout (seconds)" density="compact" hide-details/><v-text-field v-model.number="manualRunDraft.jobTimeoutSeconds" type="number" min="0" max="604800" label="Overall job deadline (0 disables)" density="compact" hide-details/><v-text-field v-model.number="manualRunDraft.retryLimit" type="number" min="0" max="10" label="Retries after failure" density="compact" hide-details/></div><div class="dispatch-note"><v-icon icon="mdi-information-outline" color="info"/>This durable dispatch is queued before execution. Closing this window does not stop it; use Cancel Run to request termination.</div><div class="window-actions"><v-btn variant="text" @click="runWindow = false">Cancel</v-btn><v-btn class="glass-button" prepend-icon="mdi-play" @click="runNow">Queue Run</v-btn></div></div></FloatingWindow>
    <FloatingWindow v-model="resultWindow" title="Live Dispatch Console" :width="720" :start-x="330" :start-y="120"><div class="result-list"><div class="dispatch-note"><v-icon :icon="dispatchRunning ? 'mdi-progress-clock' : 'mdi-check-circle-outline'" :color="dispatchRunning ? 'secondary' : 'success'"/><span>{{ dispatchRunning ? 'Run Now is active. Output is streaming as each target executes.' : 'Dispatch completed. Review the execution ledger for full reports.' }}</span><v-btn v-if="lastDispatchId" size="x-small" variant="text" prepend-icon="mdi-download" @click="downloadRunOutput">Download log</v-btn><v-btn v-if="dispatchRunning" size="x-small" color="warning" variant="text" :disabled="!dispatchId" @click="cancelRunNow">Cancel run</v-btn></div><pre class="live-output"><span v-for="item in runOutput" :key="item.id" :class="item.type">[{{ item.executionId }}] {{ item.data }}</span><span v-if="!runOutput.length" class="muted">Waiting for execution output...</span></pre><article v-for="result in runResults" :key="result.id" class="result-card"><v-icon :icon="result.status === 'success' ? 'mdi-check-circle-outline' : 'mdi-alert-circle-outline'" :color="result.status === 'success' ? 'success' : 'error'"/><div><strong>{{ result.id }}</strong><span class="mono">{{ result.status }} · exit {{ result.exit_code ?? 'pending' }}</span></div></article></div></FloatingWindow>
  </div>
</template>
