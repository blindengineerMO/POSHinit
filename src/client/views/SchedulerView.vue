...
async function runNow() { runResults.value = []; runOutput.value = []; dispatchId.value = ''; dispatchRunning.value = true; runWindow.value = false; resultWindow.value = true; try { await store.streamRunScripts(manualRunDraft, (event) => { if (event.type === 'dispatch') dispatchId.value = event.dispatchId; if (event.type === 'stdout' || event.type === 'stderr') runOutput.value.push({ ...event, id: `${event.executionId}-${runOutput.value.length}` }); if (event.type === 'complete') runResults.value.push(event.execution) }) } finally { dispatchRunning.value = false; dispatchId.value = '' } }
